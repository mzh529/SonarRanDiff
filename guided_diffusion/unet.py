...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        if use_new_attention_order:
            ...
            self.attention = QKVAttentionLegacy(self.num_heads)#在这套代码里面只调用了这个老的函数
...
class QKVAttentionLegacyQueue(nn.Module):
        
    def __init__(self, n_heads):
        super().__init__()
        self.n_heads = n_heads
        self.proj_out = zero_module(conv_nd(1, 512, 512, 1))
    ...
    def forward(self, q,k,v):
        ...
        """
        scale = 1 / math.sqrt(math.sqrt(512))
        weight = th.einsum(
...
class UNetModel_newpreview(nn.Module):
    ...
    def __init__(
        ...
        self.attentionQueue =  QKVAttentionLegacyQueue(1)
        ...
        hw = [32,16,8,4,2,1]
        self.Outreshape_blocks_localization = []
        for i in range(len(ch)):
            self.Outreshape_blocks_localization.append(nn.Sequential(
                nn.Conv2d(512, ch[i], kernel_size=1),
                nn.Upsample(scale_factor = hw[i], mode='bilinear', align_corners=True)
                ))
            
        self.Outreshape_blocks_localization = nn.ModuleList(self.Outreshape_blocks_localization)
    ...
    def extract_numbers(self,list1, list2):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        for ind, module in enumerate(self.input_blocks):
            ...
            hs.append(h)
        h = self.middle_block(h, emb)
        queue = self.extract_numbers(hs, queue)
        queue.extend(anch)
        queue = self.Uniform(queue)
        # mean_values = [np.mean(tensor) for tensor in hs]
        # print("每个张量的数值平均值:", mean_values)
        # for i in range(0, len(hs)):
        #     mean_value = np.mean(hs[i])
        #     print(mean_value)
        for i in range(1, len(hs), 3):
            random_numbers = random.sample(range(10), 3)

            temp = self.attentionQueue(queue[random_numbers[0]],queue[random_numbers[1]],queue[random_numbers[2]])
            temp = temp.view(temp.size(0),temp.size(1),int(temp.size(2)**0.5),int(temp.size(2)**0.5))
            temp = self.Outreshape_blocks_localization[int((i-1)/3)](temp)
            # temp.reshape(hs[i].shape)
            assert temp.shape == hs[i].shape
            hs[i] = hs[i] + temp
        for module in self.output_blocks:
            h = th.cat([h, hs.pop()], dim=1)            
            h = module(h, emb)
...
class Uniform(nn.Module):
    def __init__(self):
        ...
        ch = [128,128,256,256,512,512,32,64,128,256]
        mlp = [256,128,64,32,16,8,256,256,256,256]
        self.conv_blocks_localization = []
        self.linear_blocks_localization = []
        for i in range(len(ch)):
            self.conv_blocks_localization.append(nn.Conv2d(ch[i], 512, kernel_size=1))
        for i in range(len(mlp)):
            self.linear_blocks_localization.append( nn.Linear(mlp[i]**2, 64) )

        self.conv_blocks_localization = nn.ModuleList(self.conv_blocks_localization)
        self.linear_blocks_localization = nn.ModuleList(self.linear_blocks_localization)
    def forward(self, x):
        temp0=[]
        temp1=[]
        temp2=[]
        # re = [256,128,64,32,16,8,256,256,256,256]
        for i in range(len(x)):
            temp0.append(self.conv_blocks_localization[i](x[i]))
            # print(temp0[i].shape)
            temp1.append(temp0[i].view(temp0[i].size(0),temp0[i].size(1),temp0[i].size(2) * temp0[i].size(3)))
            # print(temp1[i].shape)
            temp2.append(self.linear_blocks_localization[i](temp1[i]))
        return temp2
