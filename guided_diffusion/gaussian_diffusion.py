...
#q开头的函数表示真实的，p开头的函数表示预测的
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t):
        ...
        这个函数就是计算q(x_{t-1} | x_t, x_0)的均值和方差
        """
...
#训练中的x{t-1}的预测以及采样中的x{t-1}的预测都是这个函数
    def p_mean_variance(
        ...
        if self.dpm_solver:
            ...
            for sample in self.p_sample_loop_progressive(
                ...
                if i % 5 == 0:
                    ...
                    vutils.save_image(s, fp = os.path.join('/home/mazhihao/MedSegDiff_dataDebug/res_temp_norm_6000_100', name+str(i)+".jpg"), nrow = 1, padding = 10)
...
#通过img的循环赋值，循环采样
    ...
    def _vb_terms_bpd(
        ...
        )#计算q(x_{t-1}|x_t,x_0)，也就是真实的x_{t-1}的分布
        out = self.p_mean_variance(
            model, x_t, t, clip_denoised=clip_denoised, model_kwargs=model_kwargs
        )#计算p(x_{t-1}|x_t)，也就是预测的x_{t-1}的分布
        kl = normal_kl(
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        ...
        x_t[:, -1:, ...]=res_t.float()
        terms = {}
        #res是mask二值化的结果，res_t是mask加了noise的结果；x_start是原始的图像数据输入，x_t是原始输入加了noise的结果
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.BCE_DICE or self.loss_type == LossType.RESCALED_MSE:
            ...
            terms["loss_diff"] = mean_flat((target - model_output) ** 2 )#真实噪声和预测噪声的差异loss
            terms["loss_cal"] = mean_flat((res - cal) ** 2)#预测图像掩码和真实图像掩码的差异loss
            # terms["loss_cal"] = nn.BCELoss()(cal.type(th.float), res.type(th.float)) 
            # terms["mse"] = (terms["mse_diff"] + terms["mse_cal"]) / 2.
            if "vb" in terms:
                terms["loss"] = terms["loss_diff"] + terms["vb"]#变分下界的loss，也就是扩散过程的KL(q(x_{t-1}|x_t,x_0) || p(x_{t-1}|x_t))
            else:
    ...
    def calc_bpd_loop(self, model, x_start, clip_denoised=True, model_kwargs=None):
        """
        Calculate the BPD loop for a given model and input.
        :param model: the model to evaluate loss on.
        :param x_start: the [N x C x ...] tensor of inputs.
        :param clip_denoised: if True, clip the denoised output to the valid range.
        :param model_kwargs: if not None, a dict of extra keyword arguments to
            pass to the model. This can be used for conditioning.
        :return: a tensor of shape [N] containing the BPD values for each input.
        """
        batch_size = x_start.shape[0]
        timesteps = th.arange(self.num_timesteps, device=x_start.device)
        bpd = th.zeros(batch_size, device=x_start.device)

        for t in timesteps:
            losses, _ = self.training_losses_segmentation(model, classifier, x_start, t, model_kwargs=model_kwargs)
            bpd += losses["loss"] / (t + 2)
